...
// ===== MERKEZİ VERİ =====
const N = 40, T = 32, KEY = 'cityforge1', DAY = 240000; // 1x ~= 4 dk/gün, gerçek zamana bağlı
...
let view = 'normal', hz = [], cur = null, hover = null, dirty = true, last = 0, lastDraw = 0, dpr = 1, snd = true, ac, hidden = false, cars = [];
let weather = 'sun', wT = 90000, rain = [];
...
function tickDay() { // her oyun günü: mutluluk, nüfus, ekonomi
  calc();
  const wPen = weather === 'storm' ? -4 : weather === 'rain' ? -1.5 : 0;
  const tg = 55 + wPen + st.hb.reduce((a, h) => a + h[1], 0);
...
// ===== KAYDET / YÜKLE =====
function saveGame(q) {
  try { localStorage.setItem(KEY, JSON.stringify(Object.assign({ version: 3, weather }, S))); if (!q) toast('Oyun kaydedildi.'); }
...
function roadList() { const r = []; for (let i = 0; i < S.g.length; i++) if (S.g[i] && S.g[i].k === 'road') r.push(i); return r; }
function stepCars(dt) { // basit görsel trafik: yol ağı üzerinde rastgele yürüyen araçlar, sayı trafik yoğunluğuna göre
  const rs = roadList();
  const want = Math.min(140, Math.round(rs.length * st.traffic * 1.2));
  while (cars.length < want && rs.length) { const i = rs[(Math.random() * rs.length) | 0]; cars.push({ x: i % N, y: (i / N) | 0, px: i % N, py: (i / N) | 0, t: 0, a: Math.random() * 6.28 }); }
  if (cars.length > want) cars.length = want;
  const spd = .0009 * (1.4 - st.traffic * .8);
  cars.forEach(c => {
    c.t += dt * spd;
    if (c.t >= 1) {
      c.px = c.x; c.py = c.y; c.t = 0;
      const opts = [[1, 0], [-1, 0], [0, 1], [0, -1]].filter(([a, b]) => { const n = at(c.x + a, c.y + b); return n && n.k === 'road'; });
      if (opts.length) { const [a, b] = opts[(Math.random() * opts.length) | 0]; c.a = Math.atan2(b, a); c.x += a; c.y += b; } else c.t = 0;
    }
  });
}
function drawCars(ox, oy, s) {
  if (view !== 'normal' && view !== 'traffic') return;
  const nt = .55 * (1 - (1 + Math.cos((S.t - .5) * 6.2832)) / 2);
  ctx.fillStyle = nt > .25 ? '#fde68a' : '#1e293b';
  cars.forEach(c => {
    const x = c.px + (c.x - c.px) * c.t, y = c.py + (c.y - c.py) * c.t;
    ctx.save(); ctx.translate(ox + (x + .5) * s, oy + (y + .5) * s); ctx.rotate(c.a);
    ctx.fillRect(-s * .1, -s * .05, s * .2, s * .1); ctx.restore();
  });
}
function nextWeather() { const w = ['sun', 'cloud', 'rain', 'storm']; let n; do n = w[(Math.random() * w.length) | 0]; while (n === weather); weather = n; rain = []; toast(weather === 'rain' ? '\uD83C\uDF27\uFE0F Hava yağmurlu' : weather === 'storm' ? '\u26C8\uFE0F Fırtına geliyor' : weather === 'cloud' ? '\u2601\uFE0F Hava bulutlandı' : '\u2600\uFE0F Hava açtı'); }
function stepRain(dt) {
  if (weather !== 'rain' && weather !== 'storm') { rain.length = 0; return; }
  const n = weather === 'storm' ? 90 : 45;
  while (rain.length < n) rain.push({ x: Math.random(), y: Math.random(), v: 10 + Math.random() * 8 });
  rain.forEach(d => { d.y += dt * d.v * .00004; if (d.y > 1) { d.y = 0; d.x = Math.random(); } });
}
function drawRain(W, H) {
  if (!rain.length) return;
  ctx.strokeStyle = weather === 'storm' ? 'rgba(180,200,255,.5)' : 'rgba(180,200,255,.3)'; ctx.lineWidth = dpr;
  rain.forEach(d => { const x = d.x * W, y = d.y * H; ctx.beginPath(); ctx.moveTo(x, y); ctx.lineTo(x - 3 * dpr, y + 12 * dpr); ctx.stroke(); });
}
function sane(r) { // kayıt doğrulama + sürüm geçişi (v1 → v2)
  ...
  weather = (r.weather === 'rain' || r.weather === 'storm' || r.weather === 'cloud') ? r.weather : 'sun'; delete r.weather;
...
document.addEventListener('visibilitychange', () => { hidden = document.hidden; if (!hidden) last = performance.now(); }); // arka plandan dönüşte zıplamayı önle
let sy0 = 0;
...
function loop(ts) {
  ...
  if (hidden || !Number.isFinite(dt) || dt < 0) dt = 16; // arka plana geçişte / ilk karede sıçramayı engelle
  dt = Math.min(dt, 100); // büyük lag'da gün atlamasın
  if (S.speed > 0) {
    S.t += dt * S.speed / DAY;
    if (S.t >= 1) { S.t -= 1; tickDay(); }
    wT -= dt * S.speed;
    if (wT <= 0) { wT = 60000 + Math.random() * 120000; nextWeather(); }
    stepCars(dt * S.speed); stepRain(dt * S.speed);
  }
  if (dirty || ts - lastDraw > 250) {
    lastDraw = ts;
    $('date').textContent = 'Gün ' + S.day + ' · ' + String(Math.floor(S.t * 24)).padStart(2, '0') + ':' + String(Math.floor((S.t * 24 % 1) * 60)).padStart(2, '0');
    $('wx').textContent = weather === 'rain' ? '🌧️' : weather === 'storm' ? '⛈️' : weather === 'cloud' ? '☁️' : '☀️';
    draw();
  }
